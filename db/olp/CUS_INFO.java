package com.bea.olp;

public class CUS_INFO {
    private String custId;

    private String custIdCore;

    private String custName;

    private String custType;

    private String certOrg;

    private String certType;

    private String certCode;

    private String sex;

    private String birthday;

    private String idDueDate;

    private String ifeduc;

    private String ifwedd;

    private String siaddr;

    private String sitelp;

    private String ifmobl;

    private String liveStatus;

    private String sizpcd;

    private String addr;

    private String zpcd;

    private String ifmail;

    private String relatedPersonName;

    private String relatedPersonIdNo;

    private String relatedPersonRel;

    private String relatedPersonTelp;

    private String spouseName;

    private String spouseCode;

    private String spouseType;

    private String spdbPaccFlag;

    private String resetFlag;

    private String bizOrgId;

    private String inputTime;

    private String lastUpdateTime;

    private String spousePhone;

    private String prdCodeInfo;

    private String enSurName;

    private String enGivenName;

    private String cusEname;

    private String ecifCustId;

    private String areaCode;

    private String isAlreadyLoan;

    private String blackList;

    public String getCustId() {
        return custId;
    }

    public void setCustId(String custId) {
        this.custId = custId == null ? null : custId.trim();
    }

    public String getCustIdCore() {
        return custIdCore;
    }

    public void setCustIdCore(String custIdCore) {
        this.custIdCore = custIdCore == null ? null : custIdCore.trim();
    }

    public String getCustName() {
        return custName;
    }

    public void setCustName(String custName) {
        this.custName = custName == null ? null : custName.trim();
    }

    public String getCustType() {
        return custType;
    }

    public void setCustType(String custType) {
        this.custType = custType == null ? null : custType.trim();
    }

    public String getCertOrg() {
        return certOrg;
    }

    public void setCertOrg(String certOrg) {
        this.certOrg = certOrg == null ? null : certOrg.trim();
    }

    public String getCertType() {
        return certType;
    }

    public void setCertType(String certType) {
        this.certType = certType == null ? null : certType.trim();
    }

    public String getCertCode() {
        return certCode;
    }

    public void setCertCode(String certCode) {
        this.certCode = certCode == null ? null : certCode.trim();
    }

    public String getSex() {
        return sex;
    }

    public void setSex(String sex) {
        this.sex = sex == null ? null : sex.trim();
    }

    public String getBirthday() {
        return birthday;
    }

    public void setBirthday(String birthday) {
        this.birthday = birthday == null ? null : birthday.trim();
    }

    public String getIdDueDate() {
        return idDueDate;
    }

    public void setIdDueDate(String idDueDate) {
        this.idDueDate = idDueDate == null ? null : idDueDate.trim();
    }

    public String getIfeduc() {
        return ifeduc;
    }

    public void setIfeduc(String ifeduc) {
        this.ifeduc = ifeduc == null ? null : ifeduc.trim();
    }

    public String getIfwedd() {
        return ifwedd;
    }

    public void setIfwedd(String ifwedd) {
        this.ifwedd = ifwedd == null ? null : ifwedd.trim();
    }

    public String getSiaddr() {
        return siaddr;
    }

    public void setSiaddr(String siaddr) {
        this.siaddr = siaddr == null ? null : siaddr.trim();
    }

    public String getSitelp() {
        return sitelp;
    }

    public void setSitelp(String sitelp) {
        this.sitelp = sitelp == null ? null : sitelp.trim();
    }

    public String getIfmobl() {
        return ifmobl;
    }

    public void setIfmobl(String ifmobl) {
        this.ifmobl = ifmobl == null ? null : ifmobl.trim();
    }

    public String getLiveStatus() {
        return liveStatus;
    }

    public void setLiveStatus(String liveStatus) {
        this.liveStatus = liveStatus == null ? null : liveStatus.trim();
    }

    public String getSizpcd() {
        return sizpcd;
    }

    public void setSizpcd(String sizpcd) {
        this.sizpcd = sizpcd == null ? null : sizpcd.trim();
    }

    public String getAddr() {
        return addr;
    }

    public void setAddr(String addr) {
        this.addr = addr == null ? null : addr.trim();
    }

    public String getZpcd() {
        return zpcd;
    }

    public void setZpcd(String zpcd) {
        this.zpcd = zpcd == null ? null : zpcd.trim();
    }

    public String getIfmail() {
        return ifmail;
    }

    public void setIfmail(String ifmail) {
        this.ifmail = ifmail == null ? null : ifmail.trim();
    }

    public String getRelatedPersonName() {
        return relatedPersonName;
    }

    public void setRelatedPersonName(String relatedPersonName) {
        this.relatedPersonName = relatedPersonName == null ? null : relatedPersonName.trim();
    }

    public String getRelatedPersonIdNo() {
        return relatedPersonIdNo;
    }

    public void setRelatedPersonIdNo(String relatedPersonIdNo) {
        this.relatedPersonIdNo = relatedPersonIdNo == null ? null : relatedPersonIdNo.trim();
    }

    public String getRelatedPersonRel() {
        return relatedPersonRel;
    }

    public void setRelatedPersonRel(String relatedPersonRel) {
        this.relatedPersonRel = relatedPersonRel == null ? null : relatedPersonRel.trim();
    }

    public String getRelatedPersonTelp() {
        return relatedPersonTelp;
    }

    public void setRelatedPersonTelp(String relatedPersonTelp) {
        this.relatedPersonTelp = relatedPersonTelp == null ? null : relatedPersonTelp.trim();
    }

    public String getSpouseName() {
        return spouseName;
    }

    public void setSpouseName(String spouseName) {
        this.spouseName = spouseName == null ? null : spouseName.trim();
    }

    public String getSpouseCode() {
        return spouseCode;
    }

    public void setSpouseCode(String spouseCode) {
        this.spouseCode = spouseCode == null ? null : spouseCode.trim();
    }

    public String getSpouseType() {
        return spouseType;
    }

    public void setSpouseType(String spouseType) {
        this.spouseType = spouseType == null ? null : spouseType.trim();
    }

    public String getSpdbPaccFlag() {
        return spdbPaccFlag;
    }

    public void setSpdbPaccFlag(String spdbPaccFlag) {
        this.spdbPaccFlag = spdbPaccFlag == null ? null : spdbPaccFlag.trim();
    }

    public String getResetFlag() {
        return resetFlag;
    }

    public void setResetFlag(String resetFlag) {
        this.resetFlag = resetFlag == null ? null : resetFlag.trim();
    }

    public String getBizOrgId() {
        return bizOrgId;
    }

    public void setBizOrgId(String bizOrgId) {
        this.bizOrgId = bizOrgId == null ? null : bizOrgId.trim();
    }

    public String getInputTime() {
        return inputTime;
    }

    public void setInputTime(String inputTime) {
        this.inputTime = inputTime == null ? null : inputTime.trim();
    }

    public String getLastUpdateTime() {
        return lastUpdateTime;
    }

    public void setLastUpdateTime(String lastUpdateTime) {
        this.lastUpdateTime = lastUpdateTime == null ? null : lastUpdateTime.trim();
    }

    public String getSpousePhone() {
        return spousePhone;
    }

    public void setSpousePhone(String spousePhone) {
        this.spousePhone = spousePhone == null ? null : spousePhone.trim();
    }

    public String getPrdCodeInfo() {
        return prdCodeInfo;
    }

    public void setPrdCodeInfo(String prdCodeInfo) {
        this.prdCodeInfo = prdCodeInfo == null ? null : prdCodeInfo.trim();
    }

    public String getEnSurName() {
        return enSurName;
    }

    public void setEnSurName(String enSurName) {
        this.enSurName = enSurName == null ? null : enSurName.trim();
    }

    public String getEnGivenName() {
        return enGivenName;
    }

    public void setEnGivenName(String enGivenName) {
        this.enGivenName = enGivenName == null ? null : enGivenName.trim();
    }

    public String getCusEname() {
        return cusEname;
    }

    public void setCusEname(String cusEname) {
        this.cusEname = cusEname == null ? null : cusEname.trim();
    }

    public String getEcifCustId() {
        return ecifCustId;
    }

    public void setEcifCustId(String ecifCustId) {
        this.ecifCustId = ecifCustId == null ? null : ecifCustId.trim();
    }

    public String getAreaCode() {
        return areaCode;
    }

    public void setAreaCode(String areaCode) {
        this.areaCode = areaCode == null ? null : areaCode.trim();
    }

    public String getIsAlreadyLoan() {
        return isAlreadyLoan;
    }

    public void setIsAlreadyLoan(String isAlreadyLoan) {
        this.isAlreadyLoan = isAlreadyLoan == null ? null : isAlreadyLoan.trim();
    }

    public String getBlackList() {
        return blackList;
    }

    public void setBlackList(String blackList) {
        this.blackList = blackList == null ? null : blackList.trim();
    }
}